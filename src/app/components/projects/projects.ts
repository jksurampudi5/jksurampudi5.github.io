...
export interface Project {
  ...
  role: string;
  period: string;
  startDate: string; // YYYY-MM for chronological sorting
  location: string;
  title: string;
  tagline: string;
  tools: string[];
...
export type SortMode = 'timeline-asc' | 'timeline-desc' | 'alphabetical-company';
...
export class ProjectsComponent {
  ...
  currentSort: SortMode = 'timeline-asc';

  // Companies listed in ascending chronological career order
  companies: string[] = [
    'ALL',
    'COGNIZANT',
    'THAPASSU IT GLOBAL',
  ...
  // Base list ordered in ascending chronological order (2020 -> Present)
  projects: Project[] = [
    {
      id: 'cognizant',
      company: 'COGNIZANT',
  ...
  get filteredProjects(): Project[] {
    let result = this.selectedCompany === 'ALL'
      ? [...this.projects]
      : this.projects.filter(p => p.company === this.selectedCompany);

    if (this.currentSort === 'timeline-asc') {
      result.sort((a, b) => a.startDate.localeCompare(b.startDate));
    } else if (this.currentSort === 'timeline-desc') {
      result.sort((a, b) => b.startDate.localeCompare(a.startDate));
    } else if (this.currentSort === 'alphabetical-company') {
      result.sort((a, b) => a.company.localeCompare(b.company) || a.title.localeCompare(b.title));
    }
